...
Channel.prototype._connect = function() {
	...
	if (this._connectCalled === true) {
		throw new Error('Channel object already connecting, connect cannot be called multiple times');
	}
	this._connectCalled = true;

	async.waterfall([
		function (next) {
			if (self._db._state === 'connected') {
				return next();
			}
			self._db.once('ready', next);
		},
		function (next) {
			self._db.createCollection(self.name, next);
		},
		function (col, next) {
