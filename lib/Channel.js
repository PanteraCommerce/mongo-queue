...
Channel.prototype._connect = function(db) {
	...
	async.waterfall([
		...
		function (col, next) {
			...
			self._collection.createIndex({ name: 1 }, { unique: true }, next);
		}
	], function (err) {
		if (err) {
...
Channel.prototype._publish = function (queueName, message, callback) {
	...
	this._onReady(function () {
		...
		self._collection.updateOne({ name: queueName }, { $push: { queue: message } }, { w: 1, upsert: true }, function (err) {
			callback(err);
		});
	});
	return this;
...
Channel.prototype.consume = function (queueName, callback) {
	...
	this._onReady(function () {
		callback = typeof args[args.length - 1] === 'function' ? args.pop() : noop;
		queueName = args.join(_shr.sep);
    self._collection.findOneAndUpdate({ name: queueName }, { $pop: { queue: -1 } }, { w: 1, projection: { queue: { $slice: 1 }, _id: 0 } }, function (err, doc, res) {
			if (err)
        return callback(err);
...
Channel.prototype.get = function(queueName, callback) {
	...
	this._onReady(function () {
		callback = typeof args[args.length - 1] === 'function' ? args.pop() : noop;
		queueName = args.join(_shr.sep);
		self._collection.find({ name: queueName }).project({ queue: { $slice: 1 }, _id: 0 }).toArray(function (err, doc) {
			if (err) {
				return callback(err);
			}
			doc = doc[0];
